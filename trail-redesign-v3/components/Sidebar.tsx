...
/* Oculto en mobile (< md) — ahí se usa <MobileNav /> */
  ...
  return (
    <aside className="hidden h-screen w-60 shrink-0 flex-col border-r border-stone-200/70 bg-stone-50 px-3 py-4 md:flex">
      ...
      <nav className="flex flex-col gap-0.5">
        {NAV.map(({ key, label, icon: Icon, shortcut }) => {
          ...
          return (
            ...
            >
              <Icon className={`h-4 w-4 ${isActive ? "text-stone-900" : "text-stone-400 group-hover:text-stone-600"}`} />
              <span className="flex-1 text-left">{label}</span>
              {key === "todos" && pendingCount > 0 ? (
                <span className="rounded bg-stone-200 px-1.5 py-0.5 text-[11px] font-medium tabular-nums text-stone-600">
                  {pendingCount}
                </span>
              ) : (
                shortcut && (
      ...
      <div className="flex items-center gap-2.5 rounded-md px-2 py-2 hover:bg-stone-200/40">
        <div className="flex h-7 w-7 items-center justify-center rounded-full bg-stone-200 text-[11px] font-semibold uppercase text-stone-600">
          {userName.slice(0, 2)}
        </div>
        <span className="flex-1 truncate text-sm font-medium text-stone-800">{userName}</span>
        <button
          onClick={onLogout}
