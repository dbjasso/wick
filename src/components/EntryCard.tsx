...
export type EntryTag = { name: string; color?: string | null };
      ...
      >
        ...
        {(todosTotal > 0 || tags.length > 0) && (
          <div className="mt-3 flex flex-wrap items-center gap-2">
            {todosTotal > 0 && (
              <span className="inline-flex items-center gap-1 rounded-sm bg-stone-100 px-2 py-0.5 text-[11px] font-medium tabular-nums text-stone-600">
                <CheckSquare className="h-3 w-3" />
                {todosDone}/{todosTotal}
              </span>
            )}
            {tags.map((t) => {
              const c = tagColor(t.name, t.color);
              return (
                <span
                  key={t.name}
